---
title: "ANYON MCP Server"
description: "Configure the ANYON MCP server"
---

ANYON exposes a local MCP (Model Context Protocol) server, allowing you to manage projects and tasks from external MCP clients like Claude Desktop, Raycast, or even coding agents running within ANYON itself.

<Note>
This page covers connecting **external MCP clients** to ANYON's MCP server. For configuring MCP servers **within** ANYON for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
ANYON's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the ANYON MCP server to any [supported coding agent](/supported-coding-agents) **within** ANYON.

1. In ANYON Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the ANYON card
3. Click the `Save Settings` button

<Frame>
<img src="/images/vk-mcp-server-config.jpeg" alt="MCP Servers configuration page showing how to add ANYON MCP server" />
</Frame>
...
The ANYON MCP server provides the following tools for managing projects, tasks, and task execution:
...
3. **Automatic Task Generation**: Your MCP client will use the ANYON MCP server to automatically create structured tasks in your project
...
Your MCP client will use the `create_task` tool to break this down into individual tasks and add them to your ANYON project automatically.
...
### Internal Coding Agents (Within ANYON)

A powerful workflow involves using coding agents within ANYON that are also connected to the ANYON MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual tasks directly in the ANYON interface
...
Raycast is a popular MCP client that can connect to ANYON's MCP server. Here's how to configure it:
...
<Frame>
<img src="/images/vk-raycast-mcp-part-1.png" alt="Raycast MCP configuration - adding ANYON server" />
</Frame>

Configure the ANYON MCP server in Raycast by adding the server details.
</Tab>
...
Once configured, you'll see the ANYON MCP server listed and ready to use in Raycast.
</Tab>
</Tabs>
